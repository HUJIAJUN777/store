...
条件：员工的mgr = 上级的empno
*/
SELECT t.ename,j.ename
FROM `t_employees` AS t,`t_employees` AS j
WHERE t.mgr=j.empno
...
*/
SELECT t.empno,t.ename,a.dname
FROM `t_employees` AS t,`t_employees` AS b,`t_dept` AS a
WHERE t.mgr=b.empno AND t.hiredate<b.hiredate AND a.deptno
...
WHERE a.deptno=(SELECT t.deptno FROM `t_dept` AS t WHERE t.dname='公关部')
...
FROM t_employees AS a,t_dept AS b
WHERE
a.`job`=
(SELECT t.`job`
FROM t_employees AS t
WHERE t.`ename`='张飞') AND a.`deptno`=b.`deptno`
;
